...
function makePlayer(name,x,face,cpu=false,charIndex=0){
 ...
 return{name:c.jp,x,y:G,w:92,h:140,vx:0,vy:0,hp:c.hp,maxHp:c.hp,meter:25,face,cpu,charIndex,guard:false,atk:0,type:"",active:false,stun:0,cd:0,spcd:0,throwcd:0,wins:0,airSpecial:false,down:false,downT:0,inv:0,aiT:0,aiMode:"",comboStep:0,comboT:0,guardStun:0,superState:"",superT:0,superHit:false,jumpHoldX:0,jumpHoldT:0}
}
...
function control(p){
 ...
 if((keys[U]||keys[JL]||keys[JR])&&grounded(p)&&!p.guard){
   ...
   // 着地直後の押し戻しや通常移動処理で横速度が消えないよう少しだけ保持。
   p.jumpHoldX = p.vx;
   p.jumpHoldT = 10;
 }
...
function physics(p){
 ...
 if(p.jumpHoldT>0&&!grounded(p)){p.jumpHoldT--;if(Math.abs(p.vx)<Math.abs(p.jumpHoldX))p.vx=p.jumpHoldX}
